...
def contact():
    mess._show(title='Contact us', message="Please contact us on : 'sanjilkc0@gmail.com' ")
    ...
    if ((name.isalpha()) or (' ' in name)):
        ...
        while (True):
            ...
            for (x, y, w, h) in faces:
                ...
                cv2.imwrite("TrainingImage\ " + name + "." + str(serial) + "." + VEHICLE_NUMBER + '.' + str(sampleNum) + ".jpg",
                            gray[y:y + h, x:x + w])
                # display the frame
...
def TrainImages():
    ...
    message1.configure(text=res)
    message.configure(text='Total Registrations till now  : ' + str(VEHICLE_NUMBER[0]))
...
def getImagesAndLabels(path):
    ...
    VEHICLE_NUMBERs = []
    # now looping through all the image paths and loading the VEHICLE_NUMBERs and the images
    for imagePath in imagePaths:
        ...
        VEHICLE_NUMBER = int(os.path.split(imagePath)[-1].split(".")[1])
    ...
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            if (conf < 50):
                ...
                VEHICLE_NUMBER = df.loc[df['SERIAL NO.'] == serial]['VEHICLE_NUMBER'].values
                VEHICLE_NUMBER = str(VEHICLE_NUMBER)
                VEHICLE_NUMBER = VEHICLE_NUMBER[1:-1]
                bb = str(aa)
                bb = bb[2:-2]
                presence = [str(VEHICLE_NUMBER), '', bb, '', str(date), '', str(timeStamp)]
...
trackImg = tk.Button(frame1, text="Take Presence", command=TrackImages  ,fg="black"  ,bg="#3ffc00"  ,width=35  ,height=1, activebackground = "white" ,font=('comic', 15, ' bold '))
